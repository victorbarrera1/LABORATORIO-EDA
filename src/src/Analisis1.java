...
/**
 * Análisis empírico del método cuentaPares (dos punteros sobre arreglo ordenado).
...
public class Analisis1 {
    ...
    static List<Integer> generarInstancia(int n, int t) {
        int rango = (int) Math.pow(2, t + 2); // 2^(t+2)
    ...
    public static void main(String[] args) {
        ...
        for (int t : exponentes) {
            ...
            // Semilla fija por tamaño: seed = t
            StdRandom.setSeed(t);

            // Archivo CSV: resultados_metodo1_n32.csv, etc.
            String nombreArchivo = "resultados_metodo1_n" + n + ".csv";
            Out archivo = new Out(nombreArchivo);

            // Encabezado CSV
            archivo.println("instancia,resultado,tiempo_ms");

            System.out.println("Método 1 | n=" + n + " | seed=" + t);

            for (int i = 0; i < INSTANCIAS; i++) {

                // Generar instancia (fuera de la medición)
                List<Integer> valores = generarInstancia(n, t);
                int k = StdRandom.uniformInt(1, (int) Math.pow(2, t - 2) + 1); // k en [1, 2^(t-2)]

                // Medir solo el tiempo del método
                StopwatchCPU reloj = new StopwatchCPU();
                int resultado = Result.cuentaPares(n, new ArrayList<>(valores), k);
        ...
        System.out.println("\nListo. Se generaron 6 archivos CSV para el método 1.");
    }
}
